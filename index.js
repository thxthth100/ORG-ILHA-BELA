...
function filaFiles() {
  return fs.existsSync(GEL_FILE) ? [{ attachment:GEL_FILE, name:'gelo.png' }] : [];
}
...
function buildFilaEmbed(mode, plat, value) {
  ...
  // Prioriza a foto do gelo definida por !setgelo.
  if (config.gelUrl) embed.setThumbnail(config.gelUrl);
  else if (fs.existsSync(GEL_FILE)) embed.setThumbnail('attachment://gelo.png');
  else if (config.thumbUrl) embed.setThumbnail(config.thumbUrl);
  return embed;
}
  ...
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel(`Full UMP XM8 • ${fmtVal(value)}`)
        .setEmoji(config.umpEmojiId ? { id:config.umpEmojiId } : { name:'🔫' })
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`fn_${mode}_${plat}_${v}`).setLabel(`🧊 Gel Normal • ${fmtVal(value)}`).setStyle(ButtonStyle.Secondary),
