...
function buildFilaEmbed(mode, plat, value) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle(`${mode.toUpperCase()} ${platLabel} ${fmtVal(value)}`)
...
client.on('messageCreate', async (msg) => {
  ...
  // !setticketlog #canal — define onde os transcripts serão enviados
  if (cmd === 'setticketlog') {
    if (!isAdmin) return msg.reply('❌ Só administradores podem definir os logs.');
    const channel = msg.mentions.channels.first();
    if (!channel) return msg.reply('❌ Mencione o canal. Ex: `!setticketlog #logs-ticket`');
    config.ticketLogChannel = channel.id;
    saveConfig();
    return msg.reply(`✅ Logs de tickets definidos: ${channel}`);
  }
...
async function criarTranscript(channel) {
  ...
  let ultima = null;
  try {
    while (mensagens.length < 1000) {
      const opcoes = { limit:100 };
      if (ultima) opcoes.before = ultima;
  ...
  const linhas = [
    ...
    '',
    ...mensagens.map(m => {
      const anexos = m.attachments?.size
        ? ` | Anexos: ${[...m.attachments.values()].map(a => a.url).join(', ')}`
        : '';
      return `[${new Date(m.createdTimestamp).toLocaleString('pt-BR')}] ${m.author.tag}: ${m.content || '[embed/botão]'}${anexos}`;
    }),
  ];
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (id.startsWith('ticket_close_')) {
    ...
    const partes = ticketChannel.topic?.split(':') || [];
    const donoId = partes[1];
    const nomeTranscript = `transcript-${ticketChannel.name}.txt`;
    const transcript = await criarTranscript(ticketChannel);

    // Envia para o canal de logs configurado.
    if (config.ticketLogChannel) {
      try {
        const logs = await client.channels.fetch(config.ticketLogChannel);
        await logs.send({
          content:`📄 Transcript do ticket **#${ticketChannel.name}** fechado por ${interaction.user}.`,
          files:[{ attachment:transcript, name:nomeTranscript }],
        });
      } catch (e) {
        console.error('Erro ao enviar transcript para os logs:', e);
      }
    }

    // Envia uma cópia no privado do dono do ticket.
    if (donoId) {
      try {
        const dono = await client.users.fetch(donoId);
        await dono.send({
          content:`📄 Aqui está o transcript do seu ticket **#${ticketChannel.name}**.`,
          files:[{ attachment:transcript, name:nomeTranscript }],
        });
      } catch (e) {
        console.error('Não foi possível enviar transcript no privado:', e);
      }
    }
